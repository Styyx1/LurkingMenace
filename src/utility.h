...
struct Utility
{
    ...
    inline static std::chrono::duration<double> GetTimer()
    {
        ...
        {
            auto delay =
                RandomiserUtil::GetRandomDouble(set::delay_time_min.GetValue(), set::delay_time_max.GetValue());
    ...
    inline static void RemoveAllItems(RE::TESObjectREFR *a_refToRemoveFrom, RE::TESObjectREFR *a_refToGiveItems)
    {
        ...
        for (auto &items : inv_map)
        {
            ...
            {
                a_refToRemoveFrom->GetHandle().get()->RemoveItem(
                    items.first, items.second, RE::ITEM_REMOVE_REASON::kRemove, nullptr, a_refToGiveItems);
    ...
    inline static SpawnEvent GetSpawnEvent(RE::TESObjectREFR *reference)
    {
        ...
        if (reference->GetBaseObject()->Is(RE::FormType::Container))
        {
            ...
            if (s::container_spawn_warlock_active.GetValue() &&
                (LocationCheck("LocTypeWarlockLair") || LocationCheck("LocTypeVampireLair")))
            {
                return SpawnEvent::kWarlock;
            }
            RE::PlayerCharacter *const &player = RE::PlayerCharacter::GetSingleton();
            if (s::explosion_spawn_urn.GetValue() && DoesNameContain(reference->GetName(), "urn") &&
                CellUtil::IsDungeon(ActorUtil::GetPlayerCell(player)))
            {
    ...
    inline static RE::TESNPC *GetRandomNPCFromList(std::vector<RE::TESNPC *> &vec,
                                                   RE::TESNPC *a_fallback = Forms::Loader::npc_spawn_generic)
    {
        if (vec.empty())
            return a_fallback;
    ...
    inline static RE::TESObjectREFR *PapyrusPlaceAtMe(RE::TESObjectREFR *target, RE::TESForm *object, bool persistent,
                                                      bool disabled)
    {
        if (target && object)
        {
            ...
            {
                using func_t = RE::TESObjectREFR *(*)(std::int64_t, std::int32_t, RE::TESObjectREFR *, RE::TESForm *,
                                                      std::int32_t, bool, bool);
