...
struct Settings : public REX::Singleton<Settings>
{
    ...
    inline bool ContainerEventsActive()
    {
        ...
        const auto values = {container_spawn_draugr_active.GetValue(), container_spawn_dwarven_active.GetValue(),
                             container_spawn_warlock_active.GetValue(), container_spawn_mimic_active.GetValue(),
...
struct JSONLoader : public REX::Singleton<JSONLoader>
{
    ...
    static inline json LoadJSON(const std::filesystem::path &path)
    {
        ...
        if (!file.is_open())
        {
            ...
            REX::ERROR("Could not open JSON file '{}'", pathstr);
            return {};
        }
        ...
        {
            REX::ERROR("Failed to parse '{}': {}", pathstr, e.what());
            return {};
        }
    }
    static inline void LoadExceptionsFromFolder()
    {
        ...
        {
            REX::WARN("JSON folder '{}' does not exist or is not a directory.", folderstr);
            return;
        }
        ...
        REX::INFO("Loaded all JSON exceptions: {} names, {} location keys, {} cells, {} forms", exception_names.size(),
                  exception_keywords.size(), exception_cells.size(), exception_forms.size());
    }
    ...
    {
        if (!j.contains("LocationKeys") || !j["LocationKeys"].is_array())
            return;
