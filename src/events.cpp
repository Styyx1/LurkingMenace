    ...
    EventRes LootActivateEvent::ProcessEvent(const RE::TESActivateEvent* a_event, RE::BSTEventSource<RE::TESActivateEvent>*)
    {
        ...
        if (Settings::npc_spawn_generic.GetValue() && activated_object->Is(RE::FormType::ActorCharacter)) {
            ...
            if (dead_guy && dead_guy->IsDead()) {
                auto spawn_type = Utility::GetSpawnEvent(dead_guy);
                ...
                if (npc_to_spawn && !check_timer.IsRunning()) {
                    check_timer.Start();
        ...
        if (!activated_object->GetBaseObject()->Is(RE::FormType::Container) || is_player_owned || activated_object->IsLocked())
            return EventRes::kContinue;
        ...
        if(Settings::debug_logging.GetValue())
            REX::INFO("Spawn event for container is: {}", Utility::SpawnEventToString(spawn_type));        
        ...
        // get the NPC to spawn from the type
        auto npc_to_spawn = Utility::GetNPCFromSpawnType(spawn_type);
        if (npc_to_spawn && !check_timer.IsRunning()) {
            check_timer.Start();
            // containers get disabled, NPCs don't
            bool disable_source = spawn_type <= Utility::SpawnEvent::kGeneric;
            DelayedSpawn(activated_object, npc_to_spawn, Forms::Loader::spawn_visual_explosion, Utility::GetTimer(), disable_source);
        }

        return EventRes::kContinue;
    }

    void LootActivateEvent::DelayedSpawn(RE::TESObjectREFR* source, RE::TESNPC* npc_to_spawn, RE::TESBoundObject* explosion, std::chrono::duration<double> delay, bool disable_source)
    {
        ...
        std::jthread([=] {
            ...
            SKSE::GetTaskInterface()->AddTask([=] {
                ...
                auto spawned_npc = source->PlaceObjectAtMe(npc_to_spawn, false)->AsReference();
